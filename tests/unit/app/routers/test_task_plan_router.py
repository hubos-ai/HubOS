...
from hubos.app.task_plan import PlanEventType, PlanStatus, PlanStepStatus, get_plan_store
...
async def test_create_plan_minimal(api_client: AsyncClient):
    resp = await api_client.post("/api/task-plans", json={
        "session_id": "s1",
        "title": "Test plan",
    })
    assert resp.status_code == 200
...
async def test_get_detail(api_client: AsyncClient):
    create = await api_client.post("/api/task-plans", json={
...
async def test_list_by_session_id(api_client: AsyncClient):
    await api_client.post("/api/task-plans", json={"session_id": "s1", "title": "A"})
    await api_client.post("/api/task-plans", json={"session_id": "s2", "title": "B"})
...
async def test_list_by_status(api_client: AsyncClient):
    create = await api_client.post("/api/task-plans", json={"session_id": "s1", "title": "A"})
    plan_id = create.json()["plan_id"]
    # Cancel one plan
    await api_client.post(f"/api/task-plans/{plan_id}/cancel")

    resp = await api_client.get("/api/task-plans", params={"status": "cancelled"})
    assert resp.status_code == 200
...
async def test_add_step(api_client: AsyncClient):
    create = await api_client.post("/api/task-plans", json={"session_id": "s1", "title": "P"})
    plan_id = create.json()["plan_id"]

    resp = await api_client.post(f"/api/task-plans/{plan_id}/steps", json={
        "title": "New step",
...
async def test_add_step_404(api_client: AsyncClient):
    resp = await api_client.post("/api/task-plans/nonexistent/steps", json={
        "title": "X",
    })
    assert resp.status_code == 404
...
@pytest.mark.asyncio
async def test_add_step_chat_insert_running_plan_uses_current_step(api_client: AsyncClient):
...
async def test_update_step_status_running(api_client: AsyncClient):
    ...
    add = await api_client.post(f"/api/task-plans/{plan_id}/steps", json={"title": "S"})
...
async def test_sse_receives_plan_created(api_client: AsyncClient, store):
    ...
    try:
        await api_client.post("/api/task-plans", json={
            "session_id": "s1", "title": "SSE test",
        })
