...
from hubos.config.config import (
    AgentProfileConfig,
    AgentsRunningConfig,
    load_agent_config,
    save_agent_config,
)
from hubos.constant import (
    LLM_BACKOFF_BASE,
    LLM_BACKOFF_CAP,
    LLM_MAX_RETRIES,
)
...
def mock_agent_workspace(tmp_path, monkeypatch):
    ...
    import json
    from hubos.config.utils import get_config_path
    from hubos.config.config import Config, AgentsConfig, AgentProfileRef

    # Setup workspace directory
    workspace_dir = tmp_path / "workspaces" / "test_agent"
    workspace_dir.mkdir(parents=True, exist_ok=True)

    # Patch WORKING_DIR so tests never touch the real ~/.hubos/
