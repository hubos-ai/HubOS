...
from hubos.cli.channels_cmd import channels_group
from hubos.cli.cron_cmd import cron_group
from hubos.cli.daemon_cmd import daemon_group
from hubos.cli.chats_cmd import chats_group
from hubos.cli.skills_cmd import skills_group
from hubos.config.config import AgentProfileConfig
...
def test_channels_list_default_agent(
    ...
    """Test hubos channels list uses default agent by default."""
    ...
    with patch("hubos.cli.channels_cmd.load_agent_config") as mock_load:
...
def test_channels_list_custom_agent(
    ...
    """Test hubos channels list with custom agent_id."""
...
def test_cron_list_with_agent_id():
    """Test hubos cron list with --agent-id."""
    runner = CliRunner()

    with patch("hubos.cli.cron_cmd.client") as mock_client:
...
def test_daemon_status_default_agent():
    """Test hubos daemon status defaults to 'default' agent."""
    runner = CliRunner()

    with patch("hubos.cli.daemon_cmd.run_daemon_status") as mock_status:
        with patch("hubos.cli.daemon_cmd._get_agent_workspace") as mock_ws:
...
def test_daemon_status_custom_agent():
    """Test hubos daemon status with custom agent."""
    runner = CliRunner()
...
def test_skills_list_default_agent():
    """Test hubos skills list defaults to 'default' agent."""
    runner = CliRunner()

    with patch(
        "hubos.cli.skills_cmd._get_agent_workspace",
    ) as mock_ws:
        with patch("hubos.cli.skills_cmd.SkillService") as mock_service:
            mock_ws.return_value = "/tmp/default"
...
def test_skills_list_custom_agent():
    """Test hubos skills list with custom agent."""
    runner = CliRunner()
...
def test_chats_list_with_agent_id():
    """Test hubos chats list with --agent-id."""
    runner = CliRunner()

    with patch("hubos.cli.chats_cmd.client") as mock_client:
        mock_response = MagicMock()
