...
class TestFinalResponseReviewOnly:
    ...
    def test_response_text_prefers_review_stage(self):
        ...
        with patch('hubos.core.infra.feature_flags.get_feature_flags') as mock_ff, \
            ...
            mock_llm._clean_review_output.side_effect = lambda x: x.replace("[REVIEW]", "").strip()
            ...
            stage_outputs = {
                ...
                "dev": {"content": "DEV implementation details", "confidence": 0.9},
                "review": {"content": "这是最终的用户面向答复，简洁自然。", "confidence": 0.9},
            }
            mock_llm.generate_for_stage.side_effect = lambda stage, input, context: MagicMock(
                success=True,
