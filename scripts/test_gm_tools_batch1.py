...
from hubos.core.workers.registry import set_host_agent_runner, clear_host_agent_runner  # noqa: E402
...
async def main() -> int:
    ...
    resp = await agent_workforce.spawn_subagents([
        {"agent_id": "research", "prompt": "find market size", "label": "research"},
        {"agent_id": "writer", "prompt": "draft summary", "label": "writer"},
    ...
    a4_ok = (
        ...
        and {r["label"] for r in payload["results"]} == {"research", "writer", "qa"}
    )
    report("A4 spawn_subagents 3-way fan-out all success", a4_ok, json.dumps(payload)[:300])
    ...
    a6_ok = (
        ...
        and any(
            (not r["success"]) and r["label"] == "broken" and r["error"] and "offline" in r["error"]
            for r in payload["results"]
        )
    )
    report("A6 spawn_subagents tolerates partial failure", a6_ok, json.dumps(payload)[:300])
    ...
    report("B1 recall_long_term: empty store -> friendly miss", b1_ok, _text(resp)[:200])

    # Seed: two sessions, one matches by title, one matches by message body.
    store = LocalMemoryStore()
    now = datetime.now(timezone.utc).isoformat()
    store.create_session("sess-001", {
        "session_id": "sess-001",
    ...
    store.append_message("sess-001", {
        "role": "user", "content": "what was our forecast?", "timestamp": now,
    })
    store.append_message("sess-001", {
        "role": "assistant", "content": "Forecast was 1.2M.", "timestamp": now,
    })

    store.create_session("sess-002", {
        "session_id": "sess-002",
        "title": "Travel plan",
        "started_at": now,
        "agent_id": "general",
    ...
    store.append_message("sess-002", {
        "role": "assistant", "content": "Hotel booked: Q3 marketing event in Berlin.", "timestamp": now,
    })
    ...
    store.append_message("sess-003", {
        "role": "user", "content": "completely different thing", "timestamp": now,
    })

    # B2: title-only match
    _set_ctx({"session_id": "S", "user_id": "U1", "channel": "web"})
    resp = await memory_recall.recall_long_term("budget", include_messages=False)
    payload = _json_payload(resp)
    b2_ok = (
        payload is not None
    ...
    report("B2 recall_long_term: title match (no msg search)", b2_ok, json.dumps(payload)[:300] if payload else _text(resp)[:200])
    ...
    b3_ok = (
        payload is not None
        and any(m.get("session_id") == "sess-002" and "Berlin" in (m.get("snippet") or "") for m in payload["message_hits"])
    )
    report("B3 recall_long_term: message body match", b3_ok, json.dumps(payload)[:300] if payload else _text(resp)[:200])

    # B4: user_id filter blocks other-user's session match
    resp = await memory_recall.recall_long_term("different", include_messages=True)
    payload = _json_payload(resp)
    b4_ok = (
        payload is None  # empty -> friendly text response
        or all(s["session_id"] != "sess-003" for s in (payload.get("session_hits") or []))
    )
    report("B4 recall_long_term: cross-user isolation", b4_ok, json.dumps(payload)[:300] if payload else _text(resp)[:200])

    # B5: empty query rejected
    resp = await memory_recall.recall_long_term("")
    report("B5 recall_long_term: empty query rejected", "cannot be empty" in _text(resp), _text(resp)[:200])
    ...
    report("C1 recall_session: not found returns found=False", c1_ok, json.dumps(payload)[:200] if payload else _text(resp)[:200])
    ...
    report("C2 recall_session: full retrieval", c2_ok, json.dumps(payload)[:300])
    ...
    report("C3 recall_session: truncation keeps tail", c3_ok, json.dumps(payload)[:300])
    ...
    report("C5 recall_session: empty id rejected", "cannot be empty" in _text(resp), _text(resp)[:200])
    ...
    for fname in ("agent_workforce.py", "memory_recall.py"):
        src = (SRC / "hubos" / "agents" / "tools" / fname).read_text(encoding="utf-8")
        bad = [t for t in ("openclaw", "hermes", "xclaw") if re.search(rf"\b{t}\b", src, re.IGNORECASE)]
    ...
    if failed:
        ...
        return 1
    print(f"ALL Stage B step 2c batch-1 checks passed ({len([1 for _ in []])}).")
    return 0
