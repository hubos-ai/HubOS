...
def render(label: str, resp) -> None:
    ...
    short = (
        body
        if len(body) <= 600
        else body[:600] + f" ... [{len(body)} chars total]"
    )
    print(f"  [{label}] {short}")
...
async def run() -> int:
    ...
    body1 = resp1.text
    assert (
        "Status: done" in body1
    ), f"expected Status: done, got: {body1[:200]}"
    ...
    assert (
        "Status: done" in body2t
    ), f"expected Status: done in: {body2t[:200]}"
    print("  T2 PASS")
    ...
    body3 = resp3.text
    assert (
        "not found" in body3.lower()
    ), f"expected not-found error, got: {body3}"
    print("  T3 PASS")
    ...
    render("T4", resp4)
    body4 = resp4.text
    assert ("does not yet support" in body4) or (
        "cancel will land" in body4
    ...
    results = await asyncio.gather(
        ...
        one_session(
            "e2e-S2",
            "Translate 'good morning' to French. One phrase.",
        ),
        one_session("e2e-S3", "Name one large bird. One word."),
    )
    elapsed = time.time() - t0
    print(
        f"  3 concurrent tasks finished in {elapsed:.1f}s (sequential would have been ~30s)",
    )
    ...
    if list_method:
        ...
        print(
            f"  task_store has {len(all_tasks)} tasks across sessions: "
        ...
        print(
            "  (task_store has no list method; skipping isolation introspection)",
        )
    print("  T5 PASS")
