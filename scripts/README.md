...
- Builds the console frontend (`console/`), copies `console/dist` to `src/hubos/console/dist`, then builds the wheel. Output: `dist/*.whl`.
...
- Default tag: `hubos:latest`. Uses `deploy/Dockerfile` (multi-stage: builds console then Python app).
- Example: `bash scripts/docker_build.sh myreg/hubos:v1 --no-cache`.
