...
async def main() -> int:
    ...
    report("V1 no runner -> error", "No HostAgentRunner" in _text(resp), _text(resp)[:200])
    ...
    # V2 empty plan
    resp = await agent_workforce.coordinate_workflow([])
    report("V2 empty steps rejected", "non-empty list" in _text(resp), _text(resp)[:200])

    # V3 missing fields
    resp = await agent_workforce.coordinate_workflow([{"id": "a"}])
    report("V3 missing fields rejected", "non-empty" in _text(resp), _text(resp)[:200])
    ...
    report("V4 duplicate ids rejected", "duplicate" in _text(resp), _text(resp)[:200])

    # V5 missing dep
    resp = await agent_workforce.coordinate_workflow([
        {"id": "a", "agent_id": "x", "prompt": "p", "depends_on": ["ghost"]},
    ])
    report("V5 missing dep rejected", "missing step" in _text(resp), _text(resp)[:200])
    ...
    report("V7 bad summary_step_id rejected", "summary_step_id" in _text(resp), _text(resp)[:200])

    # V8 bad id chars
    resp = await agent_workforce.coordinate_workflow(
        [{"id": "bad id!", "agent_id": "x", "prompt": "p"}],
    )
    report("V8 bad id chars rejected", "must match" in _text(resp), _text(resp)[:200])
    ...
    resp = await agent_workforce.coordinate_workflow(
        steps=[
            ...
            {"id": "B", "agent_id": "w", "prompt": "based on: {{step_A.result}}", "depends_on": ["A"]},
            {"id": "C", "agent_id": "q", "prompt": "polish: {{step_B.result}}", "depends_on": ["B"]},
        ],
        summary_step_id="C",
        title="linear",
    )
    ...
    report("E1 linear A->B->C with templating", e1_ok, json.dumps(p)[:400] if p else _text(resp)[:300])
    ...
    resp = await agent_workforce.coordinate_workflow(
        steps=[
            ...
            {"id": "B", "agent_id": "w1", "prompt": "draft from: {{step_A.result}}", "depends_on": ["A"]},
            {"id": "C", "agent_id": "w2", "prompt": "qa from: {{step_A.result}}", "depends_on": ["A"]},
            {"id": "D", "agent_id": "s", "prompt": "merge: {{step_B.result}} | {{step_C.result}}", "depends_on": ["B", "C"]},
        ],
        summary_step_id="D",
        max_concurrency=4,
    )
    ...
    resp = await agent_workforce.coordinate_workflow(
        steps=[
            ...
            {"id": "B", "agent_id": "bomb", "prompt": "q", "depends_on": ["A"]},
    ...
    report("E3 upstream fail -> downstream skipped", e3_ok, json.dumps(p)[:400] if p else _text(resp)[:300])
    ...
    report("E4 wait=False detaches", e4_ok, json.dumps(p)[:200] if p else _text(resp)[:200])
    ...
    p = _payload(resp)
    t1_ok = p is not None and p["workflow_id"] == wf_id_running and p["status"] in {"pending", "running", "done"}
    report("T1 track non-follow snapshot", t1_ok, json.dumps(p)[:200] if p else _text(resp)[:200])

    # T2 track with follow -> should complete
    resp = await agent_workforce.track_workflow(wf_id_running, follow=True, timeout_seconds=5)
    p = _payload(resp)
    t2_ok = p is not None and p["status"] == "done"
    report("T2 track follow-to-done", t2_ok, json.dumps(p)[:200] if p else _text(resp)[:200])

    # T3 track not-found
    resp = await agent_workforce.track_workflow("wf-nope")
    report("T3 track not-found error", "not found" in _text(resp), _text(resp)[:200])
    ...
    report("T4 cross-user denial on track", "access denied" in _text(resp), _text(resp)[:200])
    ...
    async def _long_runner(agent_id, prompt, ctx):
        ...
        try:
            await asyncio.wait_for(long_running_should_finish.wait(), timeout=10)
    ...
    report("C1 cancel running workflow -> cancelled", c1_ok, json.dumps(p)[:400] if p else _text(resp)[:300])
    ...
    report("C2 cancel already-terminal -> no-op", c2_ok, json.dumps(p)[:300] if p else _text(resp)[:200])

    # C3 cancel not-found
    resp = await agent_workforce.cancel_workflow("wf-nope")
    report("C3 cancel not-found error", "not found" in _text(resp), _text(resp)[:200])

    # C4 cancel empty id
    resp = await agent_workforce.cancel_workflow("")
    report("C4 cancel empty id rejected", "required" in _text(resp), _text(resp)[:200])
    ...
    import re as _re
    src_text = (SRC / "hubos" / "agents" / "tools" / "agent_workforce.py").read_text(encoding="utf-8")
    bad = [t for t in ("openclaw", "hermes", "xclaw") if _re.search(rf"\b{t}\b", src_text, _re.IGNORECASE)]
    report("Hygiene agent_workforce.py no foreign project names", not bad, f"hits: {bad}")
