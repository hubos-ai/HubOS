...
def main() -> int:
    ...
    from hubos.core.execution import get_task_store, get_event_store, get_orchestrator
    ...
    step("3. execute_task (synchronous in this probe; sa-4 will move to a thread)")
    t0 = time.time()
    try:
    ...
    if final_response:
        text = final_response if isinstance(final_response, str) else repr(final_response)[:300]
        print(f"  final_response (first 300 chars):\n    {text[:300]}")

    step("5. Inspect events in EventStore")
    events = event_store.get_events_after(task.task_id, last_seq=0) if hasattr(event_store, "get_events_after") else []
    if not events:
        # try alternative API
        for attr in ("get_events", "list_events", "get_all_events"):
            ...
            if fn:
                try:
                    events = fn(task.task_id) if attr != "get_all_events" else fn()
                    break
                except Exception:
                    continue
    print(f"  events emitted : {len(events) if hasattr(events, '__len__') else '(non-listable)'}")
