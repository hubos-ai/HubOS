...
class _StubAgentRequest:
    def __init__(self, *, session_id, user_id, input=None, channel=None) -> None:  # noqa: A002
...
_arn_agent_schemas = types.ModuleType("agentscope_runtime.engine.schemas.agent_schemas")
...
sys.modules["agentscope_runtime.engine.schemas.agent_schemas"] = _arn_agent_schemas
...
_spec = importlib.util.spec_from_file_location("host_agent_runner_under_test", _adapter_path)
...
async def main() -> int:
    ...
    # T1 happy path
    ws = _StubWorkspace(_StubRunner([
        (_StubMsg("Friday", "assistant", [_StubTextBlock("text", "hello back")]), True),
    ]))
    runner = build_host_agent_runner(_make_provider(ws))
    out = await runner("gm", "say hi", {})
    report("T1 happy path returns assistant text", out == "hello back", repr(out))

    # T2 last assistant wins (not first)
    ws2 = _StubWorkspace(_StubRunner([
    ...
    try:
        ...
        report("T3 missing runner raises RuntimeError", "no runner" in str(e), str(e))
    ...
    try:
        await build_host_agent_runner(_make_provider(_StubWorkspace(_BoomRunner())))("gm", "x", {})
        report("T4 runner exception propagates", False, "no exception")
    except ValueError as e:
        report("T4 runner exception propagates", str(e) == "boom", str(e))
    except Exception as e:  # noqa: BLE001
        report("T4 runner exception propagates", False, f"got {type(e).__name__}")
    ...
    coro = build_host_agent_runner(_make_provider(_StubWorkspace(_SlowRunner())))("gm", "x", {})
    ...
    await build_host_agent_runner(_make_provider(_StubWorkspace(captured_runner)))(
        "billing-bot",
        "refund order #42",
    ...
    msgs_ok = (
        ...
        and captured_runner.captured_msgs[0].content[0].text == "refund order #42"
    )
    ...
    report("T6 msgs passed correctly", msgs_ok, repr(captured_runner.captured_msgs))
    report("T6 AgentRequest passed correctly", req_ok, repr(req))

    # T7 AgentRequest(input=...) TypeError -> fallback
    class _StrictAgentRequest:
        def __init__(self, *, session_id, user_id, input=None, channel=None):  # noqa: A002
            if input is not None or channel is not None:
                raise TypeError("only minimal fields supported")
            self.session_id = session_id
            self.user_id = user_id

    sys.modules["agentscope_runtime.engine.schemas.agent_schemas"].AgentRequest = _StrictAgentRequest
    try:
        captured = _StubRunner([(_StubMsg("Friday", "assistant", "ok"), True)])
        out = await build_host_agent_runner(_make_provider(_StubWorkspace(captured)))("gm", "x", {"session_id": "S2"})
        ok = out == "ok" and isinstance(captured.captured_request, _StrictAgentRequest)
        report("T7 AgentRequest TypeError -> fallback works", ok, repr(captured.captured_request))
    finally:
        sys.modules["agentscope_runtime.engine.schemas.agent_schemas"].AgentRequest = _StubAgentRequest

    # T8 _extract_text shape compatibility
    cases = [
        ("dict + list[dict]", {"content": [{"type": "text", "text": "hi"}]}, "hi"),
        ("dict + str", {"content": "hi"}, "hi"),
        ("obj + list[block]", _StubMsg("a", "assistant", [_StubTextBlock("text", "hi")]), "hi"),
        # Multi-block + type filter only applies on the FALLBACK path
        # (msg has no get_text_content). Use a dict to trigger fallback.
        ("dict + multi-block, picks text-only", {"content": [
    ...
    # T9 context defaults
    captured = _StubRunner([(_StubMsg("Friday", "assistant", "ok"), True)])
    await build_host_agent_runner(_make_provider(_StubWorkspace(captured)))("gm", "p", {})
    ...
    report("T10 host adapter has no foreign project names", not banned, f"hits: {banned}")
