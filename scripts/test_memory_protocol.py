...
def _run() -> int:
    ...
    assert isinstance(store, MemoryStore), "LocalMemoryStore is not a MemoryStore"
    print("  isinstance(store, MemoryStore) = True")
    print("  T1 PASS")

    step("T2. LocalMemoryStore satisfies ArchivableMemoryStore (optional)")
    assert isinstance(store, ArchivableMemoryStore), "LocalMemoryStore lacks archive_session/auto_archive"
    print("  isinstance(store, ArchivableMemoryStore) = True")
    print("  T2 PASS")

    step("T3. LocalMemoryStore satisfies SummarizableMemoryStore (optional)")
    assert isinstance(store, SummarizableMemoryStore), "LocalMemoryStore lacks daily summary methods"
    ...
    class MinimalMockStore:
        ...
        def save_tool_call(self, session_id: str, message_id: str, tool_call: Dict[str, Any]) -> None:
            self._tools[session_id][message_id] = dict(tool_call)

        def load_session(self, session_id: str) -> Optional[Dict[str, Any]]:
            if session_id not in self._sessions:
                return None
            return {"metadata": self._sessions[session_id], "messages": self._messages[session_id]}
        ...
        def list_sessions(
            ...
            return [{"session_id": sid, **meta} for sid, meta in self._sessions.items()]
    ...
    assert isinstance(mock, MemoryStore), "MinimalMockStore failed core contract"
    assert not isinstance(mock, ArchivableMemoryStore), "MinimalMockStore should NOT satisfy archive protocol"
    assert not isinstance(mock, SummarizableMemoryStore), "MinimalMockStore should NOT satisfy summary protocol"
    # quick smoke: it does work as a MemoryStore
    mock.create_session("sess_mock", {"started_at": "2026-04-20T00:00:00", "title": "mock"})
    ...
    assert not isinstance(BrokenStore(), MemoryStore), "BrokenStore wrongly accepted as MemoryStore"
    print("  BrokenStore correctly rejected by isinstance check")
    print("  T5 PASS")

    step("T6. base.py naming hygiene")
    base_text = (SRC / "hubos" / "core" / "memory" / "base.py").read_text(encoding="utf-8")
    bad = [w for w in ("openclaw", "OpenClaw", "copaw", "CoPaw", "solo_hub") if w in base_text]
