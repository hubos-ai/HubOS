...
def _run(tmp_root: Path) -> int:
    ...
    assert get_memory_root() == tmp_root, f"env override not honored: {get_memory_root()}"
    ...
    print(f"  loaded {len(loaded['messages'])} messages, metadata.message_count={loaded['metadata']['message_count']}")
    print("  T1 PASS")

    step("T2. save_tool_call")
    store.save_tool_call(sid, "msg_001", {
        "call_id": "call_xyz",
        "message_id": "msg_001",
        "tool_name": "fake_search",
        "arguments": {"q": "hubos"},
        "result": {"status": "ok", "output": "found 0 results", "error": None},
        "timestamp": today_iso,
        "duration_ms": 12,
    })
    ...
    store.update_metadata(sid, {**loaded["metadata"], "title": "测试会话(更新后) - 项目集成"})
    store.end_session(sid, ended_at=datetime.now().isoformat(), end_reason="test_complete")
    reread = json.loads((tmp_root / "sessions" / sid / "metadata.json").read_text(encoding="utf-8"))
    ...
    assert any(r["session_id"] == sid for r in listed), f"session not in index: {listed}"
    found_by_title = store.search_sessions("测试会话")
    found_by_tag = store.search_sessions("project-hubos")
    print(f"  list_sessions={len(listed)}  search('测试会话')={len(found_by_title)}  search('project-hubos')={len(found_by_tag)}")
    ...
    assert sid_old in archived, f"expected {sid_old} in archived, got {archived}"
    # session dir gone
    ...
    assert loaded_old is not None and loaded_old["metadata"]["session_id"] == sid_old
    assert any(m.get("content") == "归档目标" for m in loaded_old["messages"])
    print(f"  archived to {archives[0].relative_to(tmp_root)}; reloaded {len(loaded_old['messages'])} msgs")
    print("  T6 PASS")
    ...
    bad_words = ["openclaw", "OpenClaw", "openClaw", "copaw", "CoPaw", "solo_hub"]
    hits = []
    ...
    print(f"  scanned {sum(1 for _ in pkg_dir.rglob('*.py'))} .py + {sum(1 for _ in pkg_dir.rglob('*.json'))} .json files: clean")
    print("  T8 PASS")
