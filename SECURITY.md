...
Report vulnerabilities of the HubOS repository:

If you discover a security issue in HubOS, please report it to us through the [Alibaba Security Response Center (ASRC)](https://security.alibaba.com/).
...
The security model and recommended baseline in this document are **aligned with** what users see and accept during **`hubos init`**. The init flow shows a security notice that covers: single-operator boundary; shared delegated authority when multiple people message the same instance; restricting channels and users (allowlists); using separate config/credentials and OS users or hosts per trust boundary; least privilege and sandboxing; keeping secrets out of the working directory and skill-accessible paths; and reviewing config and skills regularly. When updating either this document or the `SECURITY_WARNING` in `src/hubos/cli/init_cmd.py`, keep the **concepts and recommendations** consistent so operators get the same message in both places.

## Security & Trust

Security handling is owned by the HubOS maintainers. For sensitive reports, use the private advisory or a private channel as above.

## Bug Bounties

HubOS is a community open-source project. There is no bug bounty program and no budget for paid reports. Please still disclose responsibly so we can fix issues quickly. The best way to help the project right now is by sending PRs or clear, reproducible reports.

## Operator Trust Model

HubOS does **not** model one instance as a multi-tenant, adversarial user boundary.

- Authenticated callers to the same HubOS instance (same config, same channel workspace) are treated as **trusted operators** for that instance.
...
- **Recommended mode**: one user per machine/host (or per OS user), one HubOS config for that user, and one or more agents/skills inside that instance.
- If multiple users need HubOS, use one host/OS user (or VPS) per user, or strict isolation; sharing one instance by mutually untrusted users is not the recommended default.
- Skills run with the same privileges as the HubOS process; only install and enable skills you trust.


## Trusted Skills Concept

Skills/extensions are part of HubOS's **trusted computing base** for an instance.
...
- Public internet exposure of HubOS when the docs recommend against it.
- Using HubOS in ways that the docs recommend not to.
- Deployments where mutually untrusted/adversarial operators share one HubOS instance and config (e.g. reports expecting per-operator isolation for sessions, history, or similar).
...
- Exposed secrets that are third-party or user-controlled credentials (not HubOS-owned and not granting access to HubOS-operated infrastructure) without demonstrated HubOS impact.
- Scanner-only claims against stale or nonexistent paths, or without a working repro.

## Deployment Assumptions

HubOS security guidance assumes:

- The host where HubOS runs is within a trusted OS/admin boundary.
- Anyone who can modify the HubOS working directory and config (including `config.json`) is effectively a trusted operator.
...
HubOS's security model is **“personal assistant”** (one trusted operator, potentially many agents/skills), not “shared multi-tenant bus.”

- If multiple people can message the same tool-enabled HubOS instance (e.g. a shared channel workspace), they can all steer that agent within its granted permissions.
...
The HubOS working directory is treated as **trusted local operator state**.
...
- Keep secrets out of the working directory and skill-accessible paths; this is part of the baseline recommended in `hubos init`.

## Skills Trust Boundary

Skills are loaded and run **in-process** (or under the same trust boundary) as the HubOS runtime and are treated as trusted code.

- Skills can execute with the same OS privileges as the HubOS process.
...
For more operational and hardening guidance, see the [documentation](https://hubos.agentscope.io/) and any security-related docs linked from the repo.

## Runtime Requirements

- **Python**: HubOS requires a supported Python version (see [README](README.md)). Use a version with current security updates.
