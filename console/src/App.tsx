...
function AuthGuard({ children }: { children: React.ReactNode }) {
  ...
  const isDevBypass = VITE_DEV_AUTH_BYPASS === true;

  useEffect(() => {
    let cancelled = false;
    (async () => {
      // Dev bypass mode: skip /auth/* calls, require only token presence
      if (isDevBypass) {
...
function AppInner() {
  ...
  // Auto-set dev bypass token on startup so protected pages are accessible without login
  useEffect(() => {
    if (VITE_DEV_AUTH_BYPASS && !getApiToken()) {
      setAuthToken("dev-bypass-token-0000");
    }
  }, []);
