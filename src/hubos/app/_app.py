...
from ..utils.logging import setup_logger, add_hubos_file_handler
...
async def lifespan(
    ...
    add_hubos_file_handler(WORKING_DIR / "hubos.log")
    ...
    # --- hubos.core host runner injection ---
    # Wire a generic HostAgentRunner so the in-process multi-agent
    # coordination tools (spawn_subagents / coordinate_workflow / ...) can
    # dispatch work back into sibling agents managed here. The hubos.core
    # package stays host-agnostic; the adapter in hubos.integrations is what
    # bridges to MultiAgentManager.
    try:
        from hubos.core.workers import set_host_agent_runner
        from ..integrations import build_host_agent_runner

        set_host_agent_runner(
            build_host_agent_runner(multi_agent_manager.get_agent),
        )
        logger.info("hubos.core HostAgentRunner wired to MultiAgentManager")
    except Exception as exc:  # pylint: disable=broad-except
        logger.warning(
            "Failed to wire hubos.core HostAgentRunner: %s. "
            "Multi-agent coordination tools will be unavailable.",
            exc,
        )
    ...
    try:
        ...
        # Clear hubos.core host runner before tearing down MultiAgentManager so
        # in-flight coordination tools fail fast instead of hitting a stale
        # workspace provider.
        try:
            from hubos.core.workers import clear_host_agent_runner
...
# Middleware registration order — Starlette applies middleware in REVERSE
# of registration, so the outermost (first to see a request) is the LAST
# added here. Effective runtime order: CORS (if enabled) → Auth → Tenant
# → AgentContext → route handler.
#
#   AgentContextMiddleware : scopes per-agent routing state (innermost).
#   TenantContextMiddleware: binds hubos.core.TenantContext from request.state
#                            .user + X-* headers; must run AFTER Auth so
#                            request.state.user is populated.
#   AuthMiddleware         : Bearer token check; sets request.state.user.
...
# Console static dir: env, or hubos package data (console), or cwd.
_CONSOLE_STATIC_ENV = "HUBOS_CONSOLE_STATIC_DIR"


def _resolve_console_static_dir() -> str:
    if os.environ.get(_CONSOLE_STATIC_ENV):
        return os.environ[_CONSOLE_STATIC_ENV]
    # Shipped dist lives in hubos package as static data
...
def read_root():
    ...
    return {
        "message": (
            "HubOS Web Console is not available. "
            "If you installed HubOS from source code, please run "
            "`npm ci && npm run build` in HubOS's `console/` "
            "directory, and restart HubOS to enable the "
            "web console."
        ),
    }
...
@app.get("/api/version")
def get_version():
    """Return the current HubOS version."""
...
if os.path.isdir(_CONSOLE_STATIC_DIR):
    ...
    def _console_dark_logo_png():
        f = _console_path / "dark-logo.png"
        if f.is_file():
            return FileResponse(f, media_type="image/png")
        raise HTTPException(status_code=404, detail="Not Found")

    @app.get("/hubos-symbol.svg")
    def _console_icon():
        f = _console_path / "hubos-symbol.svg"
        if f.is_file():
            return FileResponse(f, media_type="image/svg+xml")
        raise HTTPException(status_code=404, detail="Not Found")
    ...
    def _console_spa(full_path: str):
        ...
        if full_path.startswith("api/") or full_path == "api":
            raise HTTPException(status_code=404, detail="Not Found")
        # Serve real static files that exist in the dist directory
        # (e.g. CHANGELOG.md, faq.zh.md) instead of falling back to index.html
