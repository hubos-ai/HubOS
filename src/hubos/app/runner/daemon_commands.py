...
Shared by in-chat /daemon <sub> and CLI `hubos daemon <sub>`.
Logs: tail WORKING_DIR / "hubos.log". Restart: in-process reload of channels,
...
async def run_daemon_restart(context: DaemonContext) -> str:
    ...
    return (
        ...
        "Run the app (e.g. `hubos app`) and use /daemon restart in chat, "
...
def run_daemon_version(context: DaemonContext) -> str:
    ...
    return (
        ...
        f"- Log file: {WORKING_DIR / 'hubos.log'}"
    )


def run_daemon_logs(lines: int = 100) -> str:
    """Tail last N lines from WORKING_DIR / hubos.log."""
    log_path = WORKING_DIR / "hubos.log"
