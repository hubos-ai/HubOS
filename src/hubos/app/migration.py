...
def _do_migrate_legacy_workspace() -> bool:
    ...
    default_agent_config = AgentProfileConfig(
        id="default",
        name="Default Agent",
        description="Default HubOS agent (migrated from legacy config)",
...
def _migrate_workspace_items_from_source(
    ...
    Args:
        source_dir: Source directory (e.g., ~/.hubos or WORKING_DIR)
...
def _do_ensure_default_agent() -> None:
    ...
    # Seed persona / role / memory / tool-guide markdown templates
    # (BOOTSTRAP / SOUL / PROFILE / AGENTS / MEMORY / HEARTBEAT).
    # Runs with skip_existing=True every startup so:
    #   - brand new installs get all templates;
    #   - legacy default workspaces (e.g. pre-rebranding installs, or first
    #     `hubos app` without `hubos init`) retroactively pick up missing
    #     template files without clobbering user-authored .md files.
    try:
        from ..agents.utils import copy_md_files

        language = (config.agents.language or "zh").split("-", 1)[0]
        ...
        if copied:
            logger.info(
                "Seeded %d default-workspace md template(s): %s",
                len(copied),
                ", ".join(copied),
            )
    except Exception as e:
        logger.warning(
            "Failed to seed default-workspace md templates: %s",
            e,
        )
...
def ensure_qa_agent_exists() -> None:
    ...
    ``hubos_source_index``), and built-in tools are restricted (see
    ``build_qa_agent_tools_config``).
...
def _do_ensure_qa_agent() -> None:
    ...
    agent_config = AgentProfileConfig(
        ...
        description=(
            "Builtin Q&A helper for HubOS setup, local config under "
            "HUBOS_WORKING_DIR, and documentation. Prefer reading files "
