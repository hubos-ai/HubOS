...
variable ``HUBOS_AUTH_ENABLED`` is set to a truthy value (``true``,
...
def is_auth_enabled() -> bool:
    ...
    Returns ``True`` when ``HUBOS_AUTH_ENABLED`` is set to a truthy
    ...
    """
    env_flag = os.environ.get("HUBOS_AUTH_ENABLED", "").strip().lower()
...
_ADMIN_ENV_VAR = "HUBOS_ADMIN_USERS"


def _parse_admin_user_env() -> frozenset[str]:
    """Read and normalise the ``HUBOS_ADMIN_USERS`` allowlist."""
    raw = os.environ.get(_ADMIN_ENV_VAR, "").strip()
    if not raw:
        return frozenset()
    return frozenset(
...
def resolve_user_roles(username: Optional[str]) -> frozenset[str]:
    """Compute the set of RBAC roles held by *username* for this request.

    Current policy, in order of precedence:

    1. **Dev / pre-install mode** — if login is disabled via
       ``HUBOS_AUTH_ENABLED`` **or** no user has been registered yet, the
       caller is trusted and granted ``admin``. This matches the
       existing :class:`AuthMiddleware` behaviour which silently skips
       auth on the same two conditions, and preserves the local-dev
       expectation that everything "just works" without a login.

    2. **Single-user mode** — when exactly one user is registered (the
       HubOS default) and the authenticated username matches that
       account, the user IS the owner and therefore the admin.

    3. **Explicit allowlist** — deployments that layer more than one
       user on top of the single-user auth (e.g. via a reverse proxy
       that injects ``X-User-Id``) can name additional admins through
       the ``HUBOS_ADMIN_USERS`` environment variable (comma-separated,
       case-insensitive).

    Anything else gets an empty role set and will fail any
    :func:`hubos.core.infra.rbac.require_roles` check.
    """
    # (1) Dev / pre-install → trusted host.
    ...
    owner_name = str(owner.get("username") or "").strip().lower()
    if owner_name and owner_name == normalised:
        return frozenset({"admin"})

    # (3) Explicit allowlist.
    if normalised in _parse_admin_user_env():
        return frozenset({"admin"})
...
def auto_register_from_env() -> None:
    ...
    Called once during application startup.  If ``HUBOS_AUTH_ENABLED``
    is truthy and both ``HUBOS_AUTH_USERNAME`` and ``HUBOS_AUTH_PASSWORD``
    ...
    username = os.environ.get("HUBOS_AUTH_USERNAME", "").strip()
    password = os.environ.get("HUBOS_AUTH_PASSWORD", "").strip()
