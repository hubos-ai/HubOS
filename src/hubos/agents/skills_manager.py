...
def _skill_config_env_var_name(skill_name: str) -> str:
    ...
    return f"HUBOS_SKILL_CONFIG_{''.join(normalized).strip('_') or 'DEFAULT'}"
...
def apply_skill_config_env_overrides(
    ...
    ``HUBOS_SKILL_CONFIG_<SKILL_NAME>`` (JSON string).
    """
...
def _extract_zip_skills(data: bytes) -> tuple[Path, list[tuple[Path, str]]]:
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="hubos_skill_upload_"))
...
def _staged_skill_dir(skill_name: str) -> Iterator[Path]:
    ...
    temp_root = Path(
        tempfile.mkdtemp(prefix=f"hubos_skill_stage_{skill_name}_"),
    )
