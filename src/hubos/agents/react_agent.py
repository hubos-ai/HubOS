# -*- coding: utf-8 -*-
"""HubOS Agent - Main agent implementation.

This module provides the main HubOSAgent class built on ReActAgent,
...
from .tools import (
    browser_use,
    cancel_task as runtime_cancel_task,
    cancel_workflow,
    coordinate_workflow,
    delegate_task,
    desktop_screenshot,
...
class HubOSAgent(ToolGuardMixin, ReActAgent):
    """HubOS Agent with integrated tools, skills, and memory management.
    ...
    Python's MRO: HubOSAgent → ToolGuardMixin → ReActAgent.  If you
    ...
    **must** call ``super()._acting(...)`` / ``super()._reasoning(...)``
    ...
    def __init__(
        ...
        # Expose request context (session_id / user_id / channel) to
        # HubOS-Runtime delegation tools via a ContextVar so they can forward
        # multi-tenant identity to Runtime. Safe to call multiple times —
        # contextvars are async-task-local.
        set_runtime_request_context(self._request_context)
    ...
    def _create_toolkit(
        ...
        tool_functions = {
            ...
            "cancel_task": runtime_cancel_task,
            # Multi-agent coordination tools (sb-2 / Stage B).
            # spawn_subagents  : stateless K-way RPC fan-out
            # coordinate_workflow / track_workflow / cancel_workflow :
            #     ad-hoc DAG across sibling agents with prompt templating
            # recall_long_term / recall_session : L4 long-term memory lookup
            # See: docs/architecture-memory-layers.md
    ...
    def _rebuild_mcp_client(client: Any) -> Any | None:
        ...
        rebuild_info = getattr(client, "_hubos_rebuild_info", None)
        ...
        try:
            if transport == "stdio":
                ...
                setattr(rebuilt_client, "_hubos_rebuild_info", rebuild_info)
                return rebuilt_client
    ...
    def _strip_tool_use_from_msg(msg: Msg) -> Msg:
        ...
        if isinstance(msg.content, str):
            msg.content += HubOSAgent._ROUND_END_NOTICE
            return msg
        ...
        filtered.append({"type": "text", "text": HubOSAgent._ROUND_END_NOTICE})
        msg.content = filtered
        return msg
    ...
    async def reply(
        ...
        logger.info("HubOSAgent.reply: max_iters=%s", self.max_iters)
