...
MANUFACTURER_KEYWORDS = {
    'manufacturer', 'factory', 'oem', 'odm', 'manufacturing',
    'producer', 'made in', 'fabricante', 'производитель', '製造',
}

DIRECT_BUYER_KEYWORDS = {
    'procurement', 'purchasing', 'import', 'importer', 'distributor',
    'wholesale', 'wholesaler', 'dealer', 'reseller', 'supplier',
    'distribution', 'school supplier', 'laboratory supplier',
}

END_USER_ONLY_KEYWORDS = {
    'kindergarten', 'primary school', 'middle school', 'high school',
    'university', 'college', 'academy', 'institute', 'campus',
}

GENERIC_LOCALS = {'info', 'contact', 'sales', 'support', 'admin', 'hello', 'office', 'vendas', 'comercial'}

# Email domains that are government/institutional — NOT business contacts
NON_BUSINESS_EMAIL_DOMAINS = {
    '.gov', '.gob', '.edu', '.ac.uk', '.k12',
    'gov.ph', 'gov.br', 'gov.mx', 'gob.mx', 'gov.ru',
    'deped.gov', 'education.gov', 'edu.ph', 'edu.br',
}
...
def load_feedback_stats():
    ...
    stats = {"bounced_emails": set(), "bounced_domains": set(), "replied_emails": set(), "replied_domains": set()}
    for r in rows:
        email = (r.get("matched_email") or r.get("from_addr") or "").lower().strip()
...
def root_brand_token(domain):
    ...
    ignored = {'www', 'en', 'eng', 'kr', 'jp', 'de', 'fr', 'ru', 'vn', 'cn', 'zh'}
...
def extract_emails_from_html(html, domain, source='raw_html'):
    ...
    seen = set()
    patterns = []
    patterns.extend(unquote(x) for x in re.findall(r'mailto:([^"\'\s>?#]+)', normalized, re.I))
    patterns.extend(m.group(1) for m in EMAIL_REGEX.finditer(normalized))
    patterns.extend(decode_cfemail(m.group(1)) for m in CFEMAIL_REGEX.finditer(normalized))
...
def crawl_single_url(target_url, domain, source='website_crawl'):
    ...
    raw_source = 'raw_html_contact_page' if source == 'website_contact_page' else 'raw_html'
    ...
    if should_use_web_crawl:
        result = call_super_crawler('web_crawl', {
            'url': target_url,
            'depth': 1,
            'spa': True
        })
        if result and result.get('success'):
            ...
            for page in pages:
                ...
                for email in page.get('emails', []) or []:
                    ...
                    if not email_obj:
                        continue
                    if not domain_matches(domain, extract_domain_from_email(email_obj['email'])):
                        continue
                    if email_obj['email'] in seen:
                        continue
                    seen.add(email_obj['email'])
    ...
    return {'emails': emails, 'title': title, 'pages': len(pages), 'visited_url': target_url}


def has_strong_email(emails):
    return any((e.get('type') == 'personal') or (float(e.get('confidence') or 0) >= 0.88) for e in emails)
...
def discover_contact_links(base_url, domain, limit=3):
    ...
    if not html:
        return []
    keywords = ('contact', 'support', 'about', 'company', 'office', 'kontakt', 'contato', 'contacto')
    links = []
    seen = set()
    for href in re.findall(r'href=["\']([^"\']+)["\']', html, re.I):
        ...
        if extract_domain(full) and not domain_matches(domain, extract_domain(full)):
            continue
        full = full.split('#', 1)[0]
        if full in seen:
            continue
...
def crawl_site_contact_data(url, domain, max_extra_targets=4):
    ...
    candidate_targets = [url] + [urljoin(base_url, path) for path in CONTACT_PATHS[:max_extra_targets]] + discovered_targets
    ...
    return {'emails': all_emails, 'title': title, 'pages': total_pages, 'visited_urls': visited_urls}
...
LOW_VALUE_DOMAINS = [
    ...
    "volza.com", "b2brazil.com", "trade.gov", "ec21.com", "globalsources.com",
    "made-in-china.com", "yellowpages-uae.com", "developmentaid.org",
    "go4worldbusiness.com", "trademo.com", "kompass.com", "industrystock.com",
    "turkishexporter.net", "saudiyellowpagesonline.com"
]

BAD_COMPANY_EXACT = {
    'school lab equipment manufacturer', 'science lab equipment', 'educational equipment supplier',
    'laboratory equipment', 'educational', 'equipment', 'home', 'welcome',
    'something went wrong', 'english', 'distribution', 'contact us', 'about us',
    'dealers and resellers', 'authorized distributor', 'find your distributor',
    'find a distributor', 'distributors', 'dealer', 'reseller', 'supplier',
    'importer', 'company', 'companies', 'english version', 'main website', 'scientific instruments',
    'laboratory & scientific equipment supplier', 'school educational equipments',
    'school lab instruments and equipments suppliers brazil manufacturers',
    'en', 'kr', 'jp', '403',
    'brazil', 'mexico', 'germany', 'japan', 'united states', 'usa', 'portugal',
    'vietnam'
}

GENERIC_COMPANY_TOKENS = {
    'lab', 'laboratory', 'education', 'educational', 'equipment', 'science', 'school',
    'supplies', 'supplier', 'importer', 'distributor', 'dealer', 'reseller', 'distribution',
    'contact', 'about', 'home', 'welcome', 'english', 'version', 'company', 'companies',
    'global', 'international', 'business', 'industrial', 'marketplace', 'directory',
    'yellowpages', 'trade', 'world', 'viet', 'vietnam', 'korea'
}

BAD_COMPANY_PHRASES = [
    'contact us', 'about us', 'find a distributor', 'find your distributor',
    'dealers and resellers', 'authorized distributor', 'school and office stationery',
    'education facility equipment', 'product details', 'tender suppliers',
    'buyers', 'company distributors', 'distributor page', 'english version',
    'discover quality', 'main website', 'scientific equipment supplier',
    'school supplies at', 'suppliers brazil manufacturers', 'the worldfolio'
]


def is_low_value_url(url):
    low = (url or "").lower()
    if low.endswith(".pdf") or "/wp-content/" in low or "/blog/" in low or "/news/" in low:
        return True
    if any(x in low for x in LOW_VALUE_DOMAINS):
        return True
    # Filter government, school, and institutional domains
    if any(d in low for d in ['.gov.', '.gob.', '.edu.', '.k12.', '.ac.uk', '.sch.']):
        return True
    # Filter known government procurement domains
    if any(d in low for d in ['depedrizal', 'deped.gov', 'lazada.com', 'shopee.']):
        return True
    low_value_paths = [
        '/buyers/', '/supplier', '/suppliers', '/distributor', '/distributors',
        '/dealers', '/resellers', '/yellowpages', '/product-details/', '/tender-suppliers'
    ]
...
def is_low_value_text(*texts):
    ...
    patterns = [
        'security checkpoint', 'vercel security checkpoint', '10 best', 'top 10',
        'best suppliers', 'best science lab equipment suppliers', 'blog', 'news',
        'article', 'directory', 'marketplace', 'yellow pages', 'dealer locator',
        'find a distributor', 'dealers and resellers', 'authorized distributor'
    ]
...
def is_invalid_company_name(name, domain=''):
    ...
    if domain:
        ...
        if any(phrase in low for phrase in ['discover quality', 'main website', 'the worldfolio']):
            return True
        if alpha_root and alpha_name and alpha_root not in alpha_name and len(tokens) >= 2 and generic_count >= max(1, len(tokens) - 1):
            return True
        if low in {'en', 'english', 'kr', 'jp', 'de', 'fr', 'ru', 'vn'}:
            return True
        if root in {'en', 'english', 'kr', 'jp', 'de', 'fr', 'ru', 'vn', 'vietnam'}:
            return True
        if domain.startswith(('en.', 'kr.', 'jp.', 'de.', 'fr.', 'ru.', 'vn.')) and len(tokens) <= 2:
            return True
        if any(bad in domain for bad in LOW_VALUE_DOMAINS) and len(tokens) <= 4:
            return True
    return False
...
def infer_role(text):
    ...
    role_map = [
        ...
        ('supplier', ['supplier']),
        ('school_or_lab_buyer', ['school', 'laboratory', 'education', 'educational', 'escolar', 'educacional']),
    ]
...
def classify_customer_fit(text):
    ...
    if end_user_hits and not any(k in low for k in ('procurement', 'purchase', 'import', 'supplier')):
        score -= 2
...
def normalize_email_details(emails_raw, domain):
    ...
    for record in emails_raw or []:
        ...
        if not domain_matches(domain, host):
            continue
        etype = record.get('type') or ('generic' if local in GENERIC_LOCALS else 'personal')
        confidence = 0.98 if etype == 'personal' else 0.82
        if local in GENERIC_LOCALS:
            confidence = min(confidence, 0.78)
        # Penalize government/institutional emails
        is_non_business = any(suffix in host for suffix in NON_BUSINESS_EMAIL_DOMAINS)
        if is_non_business:
    ...
    details.sort(key=lambda x: (x['confidence'], x['type'] == 'personal'), reverse=True)
    return details
...
def should_continue_email_enrichment(email_details):
    ...
    if best_source == 'hunter_domain' and best_type == 'personal' and best_conf >= 0.95:
        return False
...
def process_url(candidate, semaphore, feedback_stats):
    with semaphore:
        ...
        hunter_result = call_super_crawler('hunter_domain', {
            'domain': domain,
            'found_only': True
        })
        ...
        if should_continue_email_enrichment(email_details) or not company_name:
            ...
            crawl_data = crawl_site_contact_data(url, domain, max_extra_targets=max_extra_targets)

        company_name = clean_company_name(company_name or crawl_data.get('title', ''), domain)
        if not company_name:
            return None
        ...
        if crawl_data.get('emails'):
            ...
            email_details.sort(key=lambda x: (x['confidence'], x['type'] == 'personal'), reverse=True)
        primary_email, email_source, email_confidence = choose_primary_email(email_details)
        contact_name, contact_title, contact_confidence = choose_primary_contact(email_details)
        ...
        if domain in feedback_stats['bounced_domains']:
            feedback_bonus -= 3
        if primary_email and primary_email.lower() in feedback_stats['replied_emails']:
            feedback_bonus += 2
        if primary_email and primary_email.lower() in feedback_stats['bounced_emails']:
            feedback_bonus -= 4
        ...
        if is_low_value_text(company_name, candidate.get('title', ''), crawl_data.get('title', ''), company_desc):
            return None
        ...
        if crawl_data.get('emails'):
            match_reason.append(f'crawl_emails={len(crawl_data.get("emails", []))}')
...
def main():
    input_file = sys.argv[1] if len(sys.argv) > 1 else 'stage/tavily_results.json'
    output_candidates = sys.argv[2] if len(sys.argv) > 2 else 'stage/candidates_graded.json'
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = {executor.submit(process_url, c, semaphore, feedback_stats): c for c in candidates}
        for future in as_completed(futures):
            done += 1
            result = future.result()
            if result:
                results.append(result)
                print(f"[{done}/{len(candidates)}] ✅ {result['grade']} {result['buyer_role']}: {result['company_name']} | {result.get('email_pattern','-')}")
            else:
                print(f'[{done}/{len(candidates)}] ⏭️ 跳过弱候选')

    results.sort(key=lambda x: (x.get('grade') == 'A', x.get('buyer_score', 0), x.get('email_confidence', 0)), reverse=True)
    ...
    enrichment_path = Path(output_candidates).with_name('candidates_needing_enrichment.json')
