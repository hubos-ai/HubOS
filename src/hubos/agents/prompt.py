...
# Limit MEMORY.md injected into the system prompt to ~2,000 tokens.
# At roughly 4 chars/token this is 8,000 characters.
_MEMORY_MD_MAX_CHARS = 8_000
...
class PromptBuilder:
    ...
    def _load_file(self, filename: str) -> None:
        ...
        try:
            ...
            # Enforce character limit on MEMORY.md to keep token usage bounded.
            if filename == "MEMORY.md" and len(content) > _MEMORY_MD_MAX_CHARS:
                content = (
                    content[:_MEMORY_MD_MAX_CHARS]
                    + "\n\n[... memory content truncated to save tokens ...]"
                )
                logger.debug(
                    "MEMORY.md truncated to %d chars (limit: %d)",
                    _MEMORY_MD_MAX_CHARS,
                    _MEMORY_MD_MAX_CHARS,
                )
