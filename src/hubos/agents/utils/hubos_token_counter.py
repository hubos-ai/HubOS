# -*- coding: utf-8 -*-
"""Token counting utilities for HubOS using HuggingFace tokenizers.
...
class HubOSTokenCounter(HuggingFaceTokenCounter):
    """Token counter for HubOS with configurable tokenizer support.
...
class HubOSEstimateTokenCounter(HuggingFaceTokenCounter):
...
_token_counter_cache: dict[
    tuple,
    HubOSTokenCounter | HubOSEstimateTokenCounter,
] = {}


def get_hubos_token_counter(
    ...
    Args:
        ...
        use_estimate: If True (default), returns a HubOSEstimateTokenCounter
            that uses only character-based estimation without loading a
            tokenizer. If False, returns a full HubOSTokenCounter backed
    ...
    if use_estimate:
        return HubOSEstimateTokenCounter(
        ...
        if config_key not in _token_counter_cache:
            _token_counter_cache[config_key] = HubOSTokenCounter(
