...
def _normalize_working_dir_bound_paths(data: object) -> object:
    """Normalize legacy ~/.hubos-bound paths to current WORKING_DIR.

    This keeps HUBOS_WORKING_DIR effective even if user config files contain
    older hard-coded paths like "~/.hubos/media" or
    "/Users/x/.hubos/workspaces/...".
    Only rewrites known working-dir-bound keys.
    """
    legacy_root_tilde = "~/.hubos"
...
def get_available_channels() -> Tuple[str, ...]:
    ...
    hubos.channels), filtered by HUBOS_ENABLED_CHANNELS or
    HUBOS_DISABLED_CHANNELS when set.

    * HUBOS_ENABLED_CHANNELS — whitelist (only these channels are active).
    * HUBOS_DISABLED_CHANNELS — blacklist (all channels *except* these).
    * If both are set, HUBOS_ENABLED_CHANNELS takes precedence.
...
def is_running_in_container() -> bool:
    ...
    Prefer env HUBOS_RUNNING_IN_CONTAINER (1/true/yes) at call time so
