...
class VoiceChannelConfig(BaseChannelConfig):
    ...
    language: str = "en-US"
    welcome_greeting: str = "Hi! This is HubOS. How can I help you?"
...
class WeixinConfig(BaseChannelConfig):
    ...
    bot_token_file: Path to persist/load the bot_token
                    (default ~/.hubos/weixin_bot_token).
...
class AgentProfileRef(BaseModel):
    ...
    preload: bool = Field(
        default=True,
        description=(
            "Whether to preload this agent at startup. "
            "When False, the agent is loaded on first use (lazy load). "
            "Useful for secondary agents that don't need to be ready immediately."
        ),
    )
...
def load_agent_config(agent_id: str) -> AgentProfileConfig:
    ...
    # This keeps HUBOS_WORKING_DIR effective even if existing agent.json
    # contains older hard-coded paths like "~/.hubos/media".
    try:
...
def migrate_legacy_config_to_multi_agent() -> bool:
    ...
    default_agent_config = AgentProfileConfig(
        id="default",
        name="Default Agent",
        description="Default HubOS agent",
    ...
    # When HUBOS_WORKING_DIR is customized, historical data may still exist
    # under "~/.hubos".
    old_workspace = Path("~/.hubos").expanduser().resolve()
