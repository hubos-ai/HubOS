# -*- coding: utf-8 -*-
"""hubos uninstall — remove the HubOS environment and CLI wrapper."""
...
def _remove_path_entry(profile: Path) -> bool:
    """
    Remove HubOS PATH lines from a shell profile. Returns True if changed.
    """
    if not profile.is_file():
        return False

    text = profile.read_text()
    # Remove the "# HubOS" comment line and the export PATH line
    cleaned = re.sub(
        r"\n?# HubOS\nexport PATH=\"\$HOME/\.hubos/bin:\$PATH\"\n?",
        "\n",
        text,
    )
    if cleaned == text:
...
def uninstall_cmd(purge: bool, yes: bool) -> None:
    """Remove HubOS environment, CLI wrapper, and shell PATH entries."""
    wd = WORKING_DIR

    if purge:
        click.echo(f"This will remove ALL HubOS data in {wd}")
    else:
        click.echo(
            "This will remove the HubOS Python environment and CLI wrapper.",
        )
    ...
    click.echo("")
    click.echo("HubOS uninstalled. Please restart your terminal.")
