    ...
    try:
        ...
        click.echo(
            click.style(
                ...
                "Install with: pip install 'hubos[local]'",
                fg="red",
            ),
        )
...
def configure_llm_slot_interactive(*, use_defaults: bool = False) -> None:
    ...
    if not eligible:
        if use_defaults:
            click.echo(
                "No LLM provider configured. Run 'hubos models config' "
                "to configure later.",
            )
            return
        click.echo(
    ...
    if not model and use_defaults:
        click.echo(
            ...
            "Run 'hubos models config' to set one.",
        )
        return
    try:
    ...
    for defn in _all_provider_objects(manager):
        ...
        if defn.is_local:
            ...
            if all_models:
                ...
                click.echo("  Use 'hubos models download' to add models.")
        else:
...
def add_provider_cmd(
    ...
    click.echo(
        "  Run 'hubos models add-model' to add models, "
        "then 'hubos models config-key' to set the API key.",
    )
...
def download_cmd(
    ...
    \b
    Examples:
      hubos models download TheBloke/Mistral-7B-Instruct-v0.2-GGUF
      hubos models download Qwen/Qwen2-0.5B-Instruct-GGUF --source modelscope
    """
    ...
    click.echo(
        ...
        "  hubos models set-llm  (select 'hubos-local' provider)",
    )
...
def list_local_cmd() -> None:
    ...
    if not models:
        ...
        click.echo("Use 'hubos models download <repo_id>' to download one.")
        return
