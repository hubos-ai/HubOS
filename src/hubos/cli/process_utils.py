...
def _matches_hubos_cli_command(command: str, *subcommands: str) -> bool:
    """Return whether command line looks like a HubOS CLI invocation."""
    lowered = f" {command.lower()}"
    return any(
        pattern in lowered
        for subcommand in subcommands
        for pattern in (
            f" -m hubos {subcommand}",
            f" hubos {subcommand}",
            f"__main__.py {subcommand}",
            f'hubos.exe" {subcommand}',
            f"hubos.exe {subcommand}",
        )
    )


def _is_hubos_service_command(command: str) -> bool:
    """Return whether the command line looks like a local HubOS app."""
    return _matches_hubos_cli_command(command, "app")


def _is_hubos_wrapper_process(name: str, command: str) -> bool:
    """Return whether the process looks like a HubOS CLI wrapper."""
    lowered_name = name.lower().removesuffix(".exe")
    return lowered_name == "hubos" or _matches_hubos_cli_command(
        command,
        "app",
        "desktop",
    )
...
def _candidate_hosts(host: str | None) -> list[str]:
    """Return host variants that can reach a local HubOS service."""
    if not host:
        return []
