...
def chats_group() -> None:
    ...
    \b
    Common examples:
      hubos chats list                    # List all chats
      hubos chats list --user-id alice    # Filter by user
      hubos chats get <chat_id>           # View details
      hubos chats create --session-id s1 --user-id u1
      hubos chats delete <chat_id>        # Delete a chat
    """
...
def list_chats(
    ...
    Examples:
      hubos chats list
      hubos chats list --user-id alice
      hubos chats list --channel discord
      hubos chats list --user-id alice --channel discord
    """
...
def get_chat(
    ...
    \b
    CHAT_ID  Chat UUID, obtainable via `hubos chats list`.

    \b
    Examples:
      hubos chats get 823845fe-dd13-43c2-ab8b-d05870602fd8
    """
    base_url = resolve_base_url(ctx, base_url)
    with client(base_url) as c:
        headers = {"X-Agent-Id": agent_id}
...
def create_chat(
    ...
    Inline creation examples:
      hubos chats create --session-id "discord:alice" \\
    ...
    JSON file creation example:
      hubos chats create -f chat.json
    """
    base_url = resolve_base_url(ctx, base_url)
...
def update_chat(
    ...
    \b
    Examples:
      hubos chats update <chat_id> --name "Renamed Chat"
    """
    base_url = resolve_base_url(ctx, base_url)
    headers = {"X-Agent-Id": agent_id}
...
def delete_chat(
    ...
    \b
    Examples:
      hubos chats delete 823845fe-dd13-43c2-ab8b-d05870602fd8
    """
    base_url = resolve_base_url(ctx, base_url)
    with client(base_url) as c:
        headers = {"X-Agent-Id": agent_id}
