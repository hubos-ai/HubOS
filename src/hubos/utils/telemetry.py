...
TELEMETRY_ENDPOINT = "https://hubos-telemetry-xissagieap.cn-hangzhou.fcapp.run"
...
def _detect_install_method() -> str:
    """Detect how HubOS was installed based on environment signals."""
    import os

    if os.environ.get("HUBOS_RUNNING_IN_CONTAINER", "").lower() in (
        "1",
        "true",
        "yes",
    ):
        return "docker"
    if os.environ.get("HUBOS_DESKTOP_APP", "").lower() in (
        "1",
        "true",
        "yes",
    ):
...
def get_system_info() -> dict[str, Any]:
    ...
    - hubos_version: HubOS version string
    - install_method: How HubOS was installed (docker/desktop/pip)
    - os: Operating system (Windows/Darwin/Linux)
    - os_version: OS version string
    - python_version: Python version running hubos (major.minor)
    - architecture: CPU architecture (x86_64/arm64/etc)
    - has_gpu: GPU availability detection
    """
    from ..__version__ import __version__ as hubos_ver

    info = {
        "install_id": str(uuid.uuid4()),
        "hubos_version": _safe_get(lambda: hubos_ver, "unknown"),
...
def _get_current_version() -> str:
    """Get the current HubOS version string."""
    try:
        from ..__version__ import __version__ as hubos_ver
...
def has_telemetry_been_collected(working_dir: Path) -> bool:
    ...
    Re-triggers collection when HubOS is upgraded (or downgraded) to a version
    that hasn't been collected before.

    Args:
        working_dir: Path to HubOS working directory
    ...
    try:
        ...
        # v1.1 compat: single hubos_version field
        return marker_data.get("hubos_version", "") == current
    except Exception:
        return False
        ...
        if marker_file.exists():
            try:
                ...
                if not collected_versions:
                    old_ver = old_data.get("hubos_version", "")
                    if old_ver:
