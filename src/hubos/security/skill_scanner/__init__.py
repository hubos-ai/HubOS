# -*- coding: utf-8 -*-
"""
Skill security scanner for HubOS.
...
Quick start::
    ...
    from hubos.security.skill_scanner import SkillScanner
...
def _get_scan_mode(cfg: Any = None) -> str:
    ...
    Priority: env ``HUBOS_SKILL_SCAN_MODE`` > config > default ``warn``.
    """
    env = os.environ.get("HUBOS_SKILL_SCAN_MODE")
    if env is not None:
        val = env.lower().strip()
...
def _get_blocked_history_path() -> Path:
    try:
        ...
        return Path.home() / ".hubos" / _BLOCKED_HISTORY_FILE
